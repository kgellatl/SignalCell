...
tsne_xy <- embedding(sce, lem = "iPCA", embedding = "tSNE")
colnames(tsne_xy) <- c("x", "y")
colData(sce) <- cbind(colData(sce), tsne_xy)
```
...
```{r}
reducedDims(sce)
sce <- cluster_cells(sce, dims = "Comp", lem = "iPCA",  method = "spectral", k = 4)
...
c_metrics <- pivot_longer(c_metrics, cols = 1:6, values_to = "value", names_to = "metric")
c_metrics$metric <- as.factor(c_metrics$metric)
c_metrics$k <- as.factor(c_metrics$k)
