...
sce <- calc_var_genes(sce, method = "CV", threshold = 1, minCells = 10)
...
```{r}
gene_subset <- get_var_genes(sce, method = "Malhanobis", cutoff = 0.85)
sce <- dim_reduce(input = sce, genelist = gene_subset, pre_reduce = "iPCA")
sce <- embed_cells(input = sce, lem = "iPCA", method = "tSNE")
sce <- cluster_cells(sce, dims = "Comp", lem = "iPCA",  method = "spectral", num_clust = 6)
colData(sce)
```
...
# When NULL, the name of the lem defaults to the pre_reduce method, in this case iPCA
sce <- dim_reduce(input = sce, genelist = gene_subset, pre_reduce = "iPCA", reducedDim_key = NULL)
...
sce <- dim_reduce(input = sce, reducedDim_key = "PCA_20comp", genelist = gene_subset, pre_reduce = "PCA", nComp = 20)
